...
def train_and_save_models(df):
    ...
    print("\nSaving models and preprocessor to disk...")
    model_dir = 'saved_models/'
    import os
    os.makedirs(model_dir, exist_ok=True)
    
    joblib.dump(preprocessor, f'{model_dir}lead_scoring_preprocessor.joblib')
    joblib.dump(rf_model, f'{model_dir}lead_scoring_rf_model.joblib')
    joblib.dump(xgb_model, f'{model_dir}lead_scoring_xgb_model.joblib')
    
    # Save feature metadata for future reference
    model_meta = {
        'categorical_features': categorical_features,
        'numerical_features': numerical_features,
        'ensemble_weights': {'rf': 0.5, 'xgb': 0.5}  # Equal weights for ensemble
    }
    joblib.dump(model_meta, f'{model_dir}lead_scoring_metadata.joblib')
